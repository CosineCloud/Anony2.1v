...
def clean_otp(user_id):
    ...
    logger.info(f"Will clean OTP after 10 seconds")
    
    # Wait for 10 seconds (light wait)
    time.sleep(20)
    
    logger.info(f"20 seconds passed, now cleaning OTP for user {user_id}")
    ...
    try:
        ...
        if not verify_user_exists(cursor, user_id):
            conn.close()
            return
