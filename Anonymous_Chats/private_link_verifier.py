...
def verify_private_link(link_text, user_id):
    ...
    try:
        ...
        if len(link_text) < 20:  # Arbitrary minimum length to contain all required parts
            return {
                "status": "error",
                "message": "Invalid private link format. Please check and try again."
            }
        ...
        try:
            ...
            if not peer_data:
                ...
                return {
                    "status": "error",
                    "message": "Peer not found. Please check the private link and try again."
                }
            ...
            if not peer_otp or peer_otp != otp_from_link:
                ...
                return {
                    "status": "error",
                    "message": "Invalid OTP. The private link may have expired or is incorrect."
                }
            
            # Check if peer is in PRIVATE status
            if peer_status != 'PRIVATE':
                conn.close()
                return {
                    "status": "error",
                    "message": "Peer is not available for private connection."
                }
