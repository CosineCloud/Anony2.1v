...
def check_user_status(user_id):
    ...
    try:
        ...
        if existing_otp and existing_otp.strip():
            ...
            # Update status and timer if needed
            cursor.execute("""
            UPDATE users 
            SET STATUS = 'PRIVATE', TIMER = 5760
            WHERE USER_ID = ? AND (STATUS != 'PRIVATE' OR TIMER != 5760)
            """, (user_id,))
            
            if cursor.rowcount > 0:
                logger.info(f"Updated status and timer for user {user_id}")
                conn.commit()
        else:
            # Case 3: Generate new OTP and update user status
            otp = generate_otp()
            
            # Calculate OTP expiration time (current time + 10 seconds)
            otp_exp = datetime.datetime.now() + datetime.timedelta(seconds=10)
            otp_exp_str = otp_exp.strftime('%Y-%m-%d %H:%M:%S')
            
            # Update user status
            cursor.execute("""
            UPDATE users 
            SET STATUS = 'PRIVATE', TIMER = 5760, OTP = ?, OTP_EXP = ? 
            WHERE USER_ID = ?
            """, (otp, otp_exp_str, user_id))
            
            conn.commit()
            
            # Start the OTP cleanup task
            start_otp_cleanup_task(user_id)
        ...
        connection_string = f"/92{six_digit}{otp}{user_id}{five_digit}"
